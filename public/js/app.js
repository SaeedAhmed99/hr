...
window.handleFormValidation = function handleFormValidation(data) {
  var prefix = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : "";
  $('.is-invalid').removeClass('is-invalid');
  var _loop = function _loop() {
    var error_element = error;
    if (prefix != "") {
      error_element += "_".concat(prefix);
    }
    console.log(error_element);
    $("#".concat(error_element)).addClass('is-invalid');
    var messages = "";
    data.responseJSON.errors[error].forEach(function (message) {
      messages += "".concat(message);
      // messages += `<p class="m-0 p-0">${message}</p>`;
    });

    $("#".concat(error_element, "_invalid")).html(messages);
  };
...
window.modalHide = function modalHide(modal) {
  bootstrap.Modal.getInstance(document.querySelector("#".concat(modal))).hide();
};
